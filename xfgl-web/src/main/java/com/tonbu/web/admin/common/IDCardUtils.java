...
public class IDCardUtils {
    ...
    /**
     * 功能：判断字符串是否为日期格式 2020/1
     *
     * @param
     * @return
     */
    public static boolean isDaterw(String strDate) {
        String year = "[0-9]{4}";// 年
        String month = "[0-9]||0[0-9]||1[12]";// 月
        String[] rwnys = strDate.split("/");
        boolean yearf = Pattern.compile(year).matcher(rwnys[0]).matches();
        boolean monthf = Pattern.compile(month).matcher(rwnys[1]).matches();
        //Pattern pattern = Pattern.compile(rexp);
        if (yearf && monthf) {
            return true;
        } else {
...
*/
    ...
    public static void main(String[] args) {
        ...
        //末尾为x结尾的身份证号码应该写成小x
        String IDCardNum="320483199404064624";
        ...
        String mobilePhonePattern = "^((13[0-9])|(15[^4,\\D])|(18[0,5-9]))\\d{8}$";
        Pattern p = Pattern.compile(mobilePhonePattern);
        Matcher m = p.matcher("15311111111");
        ...
        String year = "[0-9]{4}";// 年
        String month = "[0-9]||0[0-9]||1[12]";// 月
        String[] rwnys = ny.split("/");
        boolean yearf = Pattern.compile(year).matcher(rwnys[0]).matches();
        boolean monthf = Pattern.compile(month).matcher(rwnys[1]).matches();
        //Pattern pattern = Pattern.compile(rexp);
        System.out.println(yearf && monthf);
