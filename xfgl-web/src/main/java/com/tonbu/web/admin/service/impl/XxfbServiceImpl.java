...
//                "       t.DETAILS ," +//查列表就不需要富文本内容了，会很慢
    ...
    public void save(Map<String, String> param) {
        ...
        if (StringUtils.isBlank(id)) {// 新增
            DBHelper.execute("INSERT INTO TB_INFORMATION(ID,TITLE,DESCRIBE,URL,TYPE,VIEWED,CREATE_DATE,CREATE_SS_ID,TUPIAN,UPDATE_TIME,STATUS,DETAILS,ISDELETE,FILE_ID,EXTERNALLY) " +
                            "VALUES(SEQ_TB_INFORMATION.NEXTVAL,?,?,?,?,0,SYSDATE,?,?,SYSDATE,?,?,'0',?,?)",
                    title,describe,url,type,u.getUser_id(),tupian,status,details,file_id,externally);
        } else {// 修改
            DBHelper.execute("UPDATE TB_INFORMATION SET TITLE=?,DESCRIBE=?,URL=?,TYPE=?,TUPIAN=?,UPDATE_TIME=SYSDATE,DETAILS=?,STATUS=?,FILE_ID=?,EXTERNALLY=? WHERE ID=?",
                    title,describe,url,type,tupian,details,status,file_id,externally,id);
        }
    ...
    public Map<String,Object>  getPageInformationById(String id){
        ...
        return    DBHelper.queryForMap("SELECT t.ID," +
                ...
                "       t.CREATE_SS_ID," +
                "       d.DEPTNAME as DWMC" +
                "  from TB_INFORMATION t left join tb_user u on u.ss_id = t.create_ss_id  LEFT JOIN SS_DEPT D  ON D.id = u.DWMC   " +
