...
@Service
public class BgtServiceImpl implements BgtService {

    @Override
    public Page<?> wtsbList(Map<String, String> param) {
        ...
        sql.append("SELECT t.ID,"+
                ...
                "       from TB_BGTWTSB t left join tb_user u on u.ss_id = t.ss_id where 1=1");
...
//        }
    ...
    public void save(Map<String, String> param) {
        ...
        if (StringUtils.isBlank(id)) {// 新增
            DBHelper.execute("INSERT INTO TB_INFORMATION(ID,TITLE,DESCRIBE,URL,TYPE,VIEWED,CREATE_DATE,CREATE_SS_ID,TUPIAN,UPDATE_TIME,STATUS,DETAILS,ISDELETE,FILE_ID,EXTERNALLY) " +
                            "VALUES(SEQ_TB_INFORMATION.NEXTVAL,?,?,?,3,0,SYSDATE,?,?,SYSDATE,?,?,'0',?,?)",
    ...
    public void bc(Map<String, String> param) {
        ...
        if (StringUtils.isBlank(id)){
            String uid = RandomUtils.getUUID();
            DBHelper.execute("INSERT INTO TB_BGTWTSB(id,title,ss_id,describe,create_time,update_time，status,isdelete) VALUES(?,?,?,?,SYSDATE,SYSDATE,?,?)",uid,title,ss_id,describe,status,0);
        }else{
            DBHelper.execute("UPDATE TB_BGTWTSB SET title=?,describe=?,update_time=SYSDATE, status=? where id = ? "
                    ,title,describe,status,id);
        }
    ...
    @Override
    public void wtsb(Map<String, String> param) {
        String id = param.get("ids");
        String[] ids = id.split("\\+");
        for (int i=0;i<ids.length;i++){
            DBHelper.execute("UPDATE TB_BGTWTSB SET ISDELETE=? where id =?",1,id);
        }
    }

    //手机端查看详情
    @Override
    public Page<?> getwtsb(Map<String, String> param) {
        String id = param.get("id");
        List<Object> args=new ArrayList<Object>();
        StringBuffer sql=new StringBuffer();
        sql.append("select t.ID,t.SS_ID,t.TITLE,t.DESCRIBE,u.NAME,t.STATUS,to_char(t.CREATE_TIME, 'yyyy/MM/dd HH24:mi:ss') AS CREATE_TIME,to_char(t.UPDATE_TIME, 'yyyy/MM/dd HH24:mi:ss') AS UPDATE_TIME " +
                "                    from TB_BGTWTSB t left join tb_user u on u.ss_id = t.ss_id where  t.ID=?");
        args.add(id);
        Limit limit = new Limit(param);
