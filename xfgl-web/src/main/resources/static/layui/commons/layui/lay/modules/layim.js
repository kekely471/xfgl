...
layui.define(['layer', 'laytpl', 'upload'], function(exports){
  ...
  //更换分组模版
  var elemGroupTpl = ['<div class="layim-add-box">'
    ,'<div class="layim-add-img"><img class="layui-circle" src="{{ d.userData.avatar }}"><p>{{ d.userData.username||"" }}</p></div>'
    ,'<div class="layim-add-remark">'
    ,'<p>选择分组</p>'
    ,'<select class="layui-select" id="LAY_imChangeGroup">'
  ...
  changeInit = function(options){
    ...
    var create = function(data){
      ...
      cache = $.extend(cache, obj);
      $("#layui-layim").html(laytpl(elemTpl).render(obj));
      layui.each(call.ready, function(index, item){
          item && item(obj);
      });
  ...
  var banRightMenu = function(){
    ...
    layimMain.find('.layim-list-friend').on('contextmenu', '.layui-layim-list li', function (e) {
      var othis = $(this);
      var id = othis[0].id;
      $("#contextmenu_" + id).parents(".layui-layim-contextmenu").remove();
      var html = '<ul id="contextmenu_' + othis[0].id + '" data-type="friend" data-id="' + othis[0].id + '" data-index="' + othis.data('index') + '">';
      html += '<li layim-event="menu_chat"><i class="layui-icon">&#xe611;</i>发送即时消息</li>';
      html += '<li layim-event="menu_moveto" data-class="'+othis[0].className+'"><i class="layui-icon" >&#xe613;</i>移动至</li></ul>';
      if (othis.hasClass('layim-null')) return;
    ...
    layimMain.find('.layim-list-friend').on('contextmenu', 'h5', function (e) {
        ...
        $("#contextmenu_" + id).parents(".layui-layim-contextmenu").remove();
        var html = '<ul id="contextmenu_' + othis[0].id + '" data-type="friend" data-id="' + othis[0].id + '" data-index="' + othis.data('index') + '">';
        html += '<li layim-event="menu_add"><i class="layui-icon">&#xe654;</i>添加分组</li>';
        html += '<li layim-event="menu_delg" data-class="'+othis[0].className+'"><i class="layui-icon" >&#xe640;</i>删除分组</li></ul>';
        if (othis.hasClass('layim-null')) return;
  ...
  var anim = 'layui-anim-upbit', events = {
    ...
    ,menu_moveto: function (othis, e) {
      var pClass = othis.data("class");
      pClass = pClass.substring(0,pClass.indexOf(" "));
      var pthis = $('.' + pClass + ':first');
      var type = pthis.data('type'), index = pthis.data('index');
      var list = pthis.attr('data-list') || pthis.index(), data = {};

      var groupList = cache.friend || {},removeGroup = cache.base['removeGroup'] || {};
      var userId = cache.mine.id || {};
      data = cache[type][index].list[list];
      var oldGroup = cache[type][index].id;

      var changeGroup = layer.open({
        type: 1
        ,title: '更换分组'
        ,shade: false
        ,area: '430px'
        ...
        ,zIndex: 66666666
        ,resize: false
        ,content: laytpl(elemGroupTpl).render({
          groupList: groupList,
          userData: data
        })
        ,btn: ['确认', '取消']
        ,yes: function(index, layero){
          var newGroup = layero.find('#LAY_imChangeGroup').val();
          $.post(removeGroup.url, {"userId": userId,"friendId": data.id, "oldType": oldGroup , "newType": newGroup}, function (result) {
            layer.close(changeGroup);
            changeInit(cache.base);
          })
        }
      });
    }
    ...
    //添加分组
    ,menu_add: function (othis, e) {
      var userId = cache.mine.id || {};
      var addGroup = cache.base['addGroup'] || {};
      var addGroups = layer.open({
          type: 1
          ,title: '新建分组'
          ,shade: false
          ,area: '310px'
          ...
          ,zIndex: 66666666
          ,resize: false
          ,content: '<div class="layim-add-box"><div class="layim-add-remark">' +
            '<textarea id="LAY_imAddGroup" placeholder="分组名称" class="layui-textarea"></textarea>' +
            '</div></div>'
          ,btn: ['确认', '取消']
          ,yes: function(index, layero){
            var newGroup = layero.find('#LAY_imAddGroup').val();
            $.post(addGroup.url, {"userId": userId,"typeName": newGroup}, function (result) {
              layer.close(addGroups);
              changeInit(cache.base);
            })
          }
      });
