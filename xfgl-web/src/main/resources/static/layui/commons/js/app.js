            ...
            if (_config.type === 'iframe') {
                tab.set({
                    ...
                    onSwitch: function(data) { //选项卡切换时触发
                        $(".layui-tab-content .layui-tab-item").each(function(index){
                            if($(this).attr("lay-item-id") == data.layId){
                                try {
                                    $(".layui-tab-content .layui-tab-item").eq(index).find("iframe")[0].contentWindow.refreshTable();
                                }
                                catch(err){
                                }
                            }
